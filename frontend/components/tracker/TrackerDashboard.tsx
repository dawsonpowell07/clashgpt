...
export function TrackerDashboard({
  ...
  useEffect(() => {
    ...
    authFetch(`${API_URL}/api/tracker/me/battles?page=${page}&page_size=20`)
  ...
  return (
    <div className="w-full space-y-8">
      ...
      {(hasData || loadingDecks) && (
        <section className="space-y-3">
          <h2 className="text-sm font-semibold uppercase tracking-widest text-muted-foreground/70 flex items-center gap-2">
            <Zap className="w-4 h-4" /> Your Top Decks
          </h2>
          {loadingDecks ? (
      ...
      {(hasData || loadingBattles) && (
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold uppercase tracking-widest text-muted-foreground/70 flex items-center gap-2">
              <Swords className="w-4 h-4" /> Battle History
            </h2>
              ...
              {battlesData.total_pages > 1 && (
                <div className="flex items-center justify-center gap-3 pt-2">
                  <button
                    ...
                    className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-border text-sm text-muted-foreground hover:bg-muted/40 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                  >
                    <ChevronLeft className="w-3.5 h-3.5" /> Prev
                  </button>
                  <span className="text-xs text-muted-foreground">
                    {page} / {battlesData.total_pages}
                  </span>
                  <button
