...
function DeckGrid({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {playerName && (
        <p className="text-xs text-muted-foreground text-center truncate">{playerName}</p>
      )}
    </div>
  );
}
...
export default function BattleDetailPage() {
  ...
  useEffect(() => {
    ...
    getToken()
      .then((token) =>
        fetch(`${API_URL}/api/tracker/battles/${encodeURIComponent(battleId)}`, {
  ...
  }, [isLoaded, isSignedIn, getToken, battleId]);
        ...
        {isLoaded && !isSignedIn && (
          <div className="rounded-2xl border border-border/50 bg-card/40 p-10 text-center space-y-3">
            <p className="font-[family-name:var(--font-heading)] text-lg font-bold text-foreground">
              Sign in to view battle details
            </p>
            <p className="text-sm text-muted-foreground">
              You need to be signed in to view your battle history.
            </p>
          </div>
        )}
            ...
            <div className="rounded-2xl border border-border/50 bg-card/40 p-5 sm:p-7">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6">
                ...
                <div className="flex flex-col gap-2 bg-muted/30 border border-border/40 rounded-xl p-4 sm:p-5">
                  ...
                  <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">Elixir Leaked</p>
