...
const DECKS_URL = "/api/backend/api/seasonal-ladder/decks";
...
function SeasonalLadderContent() {
  ...
  useEffect(() => {
    if (SEASONAL_LADDER_CONFIG.enabled && !hasFetchedRef.current) {
  ...
  if (!SEASONAL_LADDER_CONFIG.enabled) {
    return (
      <div
        className={cn(
          inter.className,
      ...
      >
        ...
        <div className="text-center space-y-4 relative z-10 px-4">
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-blue-500/90">
              Seasonal Ladder
            </p>
          </div>
          ...
          <p className="text-sm text-muted-foreground max-w-sm mx-auto leading-relaxed">
            There is no seasonal ladder mode running right now. Check back at
  ...
  return (
    ...
    >
      <div className="fixed inset-0 hexagon-pattern opacity-[0.03] pointer-events-none" />
      <div className="fixed top-0 left-1/4 w-96 h-96 bg-blue-500/8 rounded-full blur-3xl pointer-events-none" />
      <div className="fixed bottom-0 right-1/4 w-96 h-96 bg-cyan-500/6 rounded-full blur-3xl pointer-events-none" />

      <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 relative z-10">
        {/* Header */}
        <div className="relative">
          <div className="w-12 h-0.5 bg-blue-500 mb-4 rounded-full" />
            ...
            <div>
              ...
              <h1 className="font-[family-name:var(--font-heading)] text-5xl sm:text-7xl font-extrabold tracking-tight leading-none">
                <span className="bg-gradient-to-br from-foreground via-foreground/90 to-foreground/60 bg-clip-text text-transparent">
                  {SEASONAL_LADDER_CONFIG.name}
                </span>
                <br />
                <span className="bg-gradient-to-r from-blue-400 via-cyan-300 to-blue-500 bg-clip-text text-transparent">
                  {SEASONAL_LADDER_CONFIG.subtitle}
                </span>
              </h1>
            </div>
            <div className="flex flex-col gap-2 pb-1 max-w-sm">
              <p className="text-sm text-muted-foreground leading-relaxed">
                {SEASONAL_LADDER_CONFIG.description}
              </p>
            </div>
          </div>

          <div className="mt-6 h-px bg-gradient-to-r from-blue-500/40 via-border/50 to-transparent" />
        </div>
        ...
        {/* Rate Limit Warning */}
        {rateLimitError && (
          <div className="flex items-center gap-3 px-5 py-4 rounded-xl bg-blue-500/10 border border-blue-500/30 text-blue-600 dark:text-blue-400">
            <svg
...
export default function SeasonalLadderPage() {
  return (
    <Suspense>
      <SeasonalLadderContent />
    </Suspense>
  );
}
