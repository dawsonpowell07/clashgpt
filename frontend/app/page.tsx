...
const SAMPLE_DECKS = [
  {
    ...
    winRate: 54.2,
    cards: [
      { name: "Mortar", variant: "evolved", image: "/cards/mortar/mortar_evolution.png" },
      { name: "Skeleton Barrel", variant: "evolved", image: "/cards/skeleton_barrel/skeleton_barrel_evolution.png" },
      { name: "Knight", variant: "hero", image: "/cards/knight/knight_hero.png" },
      { name: "Goblins", variant: "hero", image: "/cards/goblins/goblins_hero.png" },
      { name: "Fireball", variant: "normal", image: "/cards/fireball/fireball.png" },
      { name: "the log", variant: "normal", image: "/cards/the_log/the_log.png" },
      { name: "Minions", variant: "normal", image: "/cards/minions/minions.png" },
      { name: "Dart Goblin", variant: "normal", image: "/cards/dart_goblin/dart_goblin.png" },
    ],
  },
  {
    ...
    winRate: 51.8,
    cards: [
      { name: "Royal Hogs", variant: "evolved", image: "/cards/royal_hogs/royal_hogs_evolution.png" },
      { name: "Royal Ghost", variant: "evolved", image: "/cards/royal_ghost/royal_ghost_evolution.png" },
      { name: "Wizard", variant: "hero", image: "/cards/wizard/wizard_hero.png" },
      { name: "Goblins", variant: "hero", image: "/cards/goblins/goblins_hero.png" },
      { name: "Lightning", variant: "normal", image: "/cards/lightning/lightning.png" },
      { name: "Barbarian Barrel", variant: "normal", image: "/cards/barbarian_barrel/barbarian_barrel.png" },
      { name: "Goblin Hut", variant: "normal", image: "/cards/goblin_hut/goblin_hut.png" },
      { name: "Skeletons", variant: "normal", image: "/cards/skeletons/skeletons.png" },
    ],
  },
];
      ...
      <div className="flex-1 p-4 space-y-3 overflow-hidden">
        <div className="flex justify-end">
          <div className="bg-primary/15 border border-primary/25 rounded-xl rounded-br-sm px-3 py-2 max-w-[85%]">
            <p className="text-xs text-foreground">What&apos;s the best counter to Hog Rider?</p>
          </div>
        </div>
        <div className="flex justify-start max-w-[90%]">
          <div className="bg-muted/30 border border-border/40 rounded-xl rounded-bl-sm px-3 py-2">
            <p className="text-xs text-muted-foreground leading-relaxed">
              Based on live battle data, <span className="text-primary font-medium">Cannon</span> and <span className="text-primary font-medium">Mini P.E.K.K.A</span> are the highest win-rate counters at 58%+...
            </p>
          </div>
        </div>
        <div className="flex justify-end">
          <div className="bg-primary/15 border border-primary/25 rounded-xl rounded-br-sm px-3 py-2 max-w-[85%]">
            <p className="text-xs text-foreground">Show me top decks with Hog Rider evo</p>
          </div>
        </div>
        <div className="flex items-center gap-2 px-3 py-2 bg-primary/10 border border-primary/20 rounded-lg">
          <div className="w-3 h-3 border border-primary/60 border-t-primary rounded-full animate-spin shrink-0" />
          <span className="text-xs text-primary/80">Searching deck database…</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 mb-3">
        <div className="text-xs font-bold text-foreground/60 uppercase tracking-widest">Win Rate</div>
        <div className="ml-auto text-2xl font-black text-emerald-400 tabular-nums" style={{ fontFamily: "var(--font-heading)" }}>53.2%</div>
      </div>
      <div className="h-1.5 rounded-full bg-muted/40 overflow-hidden mb-4">
        <div className="h-full rounded-full bg-emerald-400" style={{ width: "53.2%" }} />
      </div>
      {matchups.map((m) => (
        <div key={m.vs} className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs ${m.good ? "border-emerald-500/20 bg-emerald-500/5" : "border-rose-500/15 bg-rose-500/5"}`}>
          <span className={`font-bold tabular-nums w-10 ${m.good ? "text-emerald-400" : "text-rose-400"}`}>{m.wr}%</span>
          <span className="text-foreground/70 flex-1 truncate">{m.vs}</span>
          <span className="text-muted-foreground/50 text-[10px]">{m.games}g</span>
        </div>
      ))}
    </div>
  );
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        ...
        <div className="ml-auto px-2 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-md">
          <span className="text-[10px] font-bold text-emerald-400">Legend I</span>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div key={s.label} className="bg-muted/20 border border-border/30 rounded-lg p-2 text-center">
      ...
      <div className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60 mt-1">Most Used Deck</div>
      <div className="grid grid-cols-8 gap-1">
        {["hog_rider", "musketeer", "ice_golem", "fireball", "the_log", "skeletons", "ice_spirit", "cannon"].map((card, i) => (
          <div key={i} className="aspect-[3/4] rounded-md bg-muted/40 border border-border/30 overflow-hidden relative">
            <Image src={`/cards/${card}/${card}.png`} alt={card} fill className="object-contain p-px" />
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-1.5 mb-3">
        <div className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" />
        <span className="text-[10px] text-green-400 font-medium uppercase tracking-widest">Live Tracking</span>
      </div>
      {players.map((p) => (
        <div key={p.tag} className="flex items-center gap-2.5 px-3 py-2 bg-muted/20 border border-border/30 rounded-lg">
          ...
          <div className={`text-xs font-bold tabular-nums ${p.up ? "text-emerald-400" : "text-rose-400"}`}>{p.change}</div>
        </div>
      ))}
    </div>
  );
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2">
        <div className="bg-primary/10 border border-primary/25 rounded-lg p-2.5 text-center">
          <div className="text-[10px] text-muted-foreground mb-1 uppercase tracking-wider">Win Condition A</div>
        ...
        <div className="bg-accent/10 border border-accent/25 rounded-lg p-2.5 text-center">
          <div className="text-[10px] text-muted-foreground mb-1 uppercase tracking-wider">Win Condition B</div>
      ...
      <div className="h-1.5 rounded-full overflow-hidden bg-muted/30 flex">
        <div className="h-full bg-emerald-400 rounded-l-full" style={{ width: "54.8%" }} />
        <div className="h-full bg-rose-400 rounded-r-full flex-1" />
      </div>
      <div className="text-center">
        <span className="text-[10px] text-muted-foreground">Based on </span>
        <span className="text-[10px] font-semibold text-foreground">2,430 recorded battles</span>
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative min-h-[88vh] flex flex-col items-center justify-center px-6 md:px-12 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          ...
          <div className="absolute bottom-40 left-1/4 w-1 h-1 bg-primary/20 rounded-full animate-pulse" style={{ animationDelay: "2s" }} />
          <div className="absolute top-1/3 right-1/3 w-1.5 h-1.5 bg-accent/25 rounded-full animate-pulse" style={{ animationDelay: "1.5s" }} />
          <div className="absolute top-60 left-1/3 w-1 h-1 bg-primary/15 rounded-full animate-pulse" style={{ animationDelay: "0.7s" }} />
        </div>
        ...
        <div className="relative z-10 max-w-5xl w-full text-center space-y-8 opacity-0 animate-[fade-in_1s_ease-out_0.2s_forwards]">
          ...
          <p className="text-base md:text-lg text-muted-foreground/80 max-w-2xl mx-auto leading-relaxed">
            Real-time analytics, AI chat, matchup analysis, player profiles, and meta tracking — all in one place.
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
            {[
              ...
              <div key={f.label} className="inline-flex items-center gap-1.5 px-3 py-1 bg-card/40 border border-border/40 rounded-full backdrop-blur-sm">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16 space-y-4 opacity-0 animate-[fade-in_0.8s_ease-out_0.2s_forwards]">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Every insight you need to dominate Clash Royale, from AI-powered chat to deep battle analytics.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 opacity-0 animate-[fade-in_0.8s_ease-out_0.4s_forwards]">
            ...
            <Link href="/chat" className="group lg:col-span-2 relative bg-card border border-border/50 rounded-2xl overflow-hidden hover:border-primary/30 hover:shadow-[0_0_40px_rgba(255,159,28,0.08)] transition-all duration-300 hover:-translate-y-0.5">
                ...
                <div className="flex items-start justify-between mb-5">
                  <div className="space-y-2">
                    <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-primary/10 border border-primary/30 rounded-full">
                      ...
                      <span className="text-[10px] font-semibold text-primary uppercase tracking-widest">AI Chat</span>
                    </div>
                    <h3 className="font-[family-name:var(--font-heading)] text-2xl font-bold text-foreground">
                      Ask Anything About<br />Clash Royale
                    </h3>
                    <p className="text-sm text-muted-foreground max-w-sm leading-relaxed">
                      Conversational AI grounded in live data. Get deck recommendations, player analysis, and meta insights in seconds.
                    </p>
                  </div>
            ...
            {/* Matchup Analyzer */}
            <Link href="/matchups" className="group relative bg-card border border-border/50 rounded-2xl overflow-hidden hover:border-emerald-500/30 hover:shadow-[0_0_30px_rgba(52,211,153,0.06)] transition-all duration-300 hover:-translate-y-0.5">
                ...
                <div className="flex items-start justify-between mb-5">
                  <div className="space-y-2">
                    <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/30 rounded-full">
                      ...
                      <span className="text-[10px] font-semibold text-emerald-400 uppercase tracking-widest">Matchup Analyzer</span>
                    </div>
                    <h3 className="font-[family-name:var(--font-heading)] text-xl font-bold text-foreground">
                      Analyze Any Deck&apos;s Matchups
                    </h3>
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Select 8 cards to see head-to-head win rates against every opponent deck in our database.
                    </p>
                  </div>
            ...
            {/* Player Profiles */}
            <Link href="/profiles" className="group relative bg-card border border-border/50 rounded-2xl overflow-hidden hover:border-accent/30 hover:shadow-[0_0_30px_rgba(80,120,200,0.06)] transition-all duration-300 hover:-translate-y-0.5">
                ...
                <div className="flex items-start justify-between mb-5">
                  <div className="space-y-2">
                    <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-accent/10 border border-accent/30 rounded-full">
                      ...
                      <span className="text-[10px] font-semibold text-accent uppercase tracking-widest">Player Profiles</span>
                    </div>
                    <h3 className="font-[family-name:var(--font-heading)] text-xl font-bold text-foreground">
                      Player Breakdowns
                    </h3>
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Search top-ranked players. View their stats, most-played decks, and recent battle history.
                    </p>
                  </div>
            ...
            {/* Deck Browser */}
            <Link href="/decks" className="group relative bg-card border border-border/50 rounded-2xl overflow-hidden hover:border-primary/30 hover:shadow-[0_0_30px_rgba(255,159,28,0.06)] transition-all duration-300 hover:-translate-y-0.5">
              ...
              <div className="p-6 flex flex-col h-full">
                <div className="flex items-start justify-between mb-5">
                  <div className="space-y-2">
                    <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-primary/10 border border-primary/30 rounded-full">
                      ...
                      <span className="text-[10px] font-semibold text-primary uppercase tracking-widest">Deck Browser</span>
                    </div>
                    <h3 className="font-[family-name:var(--font-heading)] text-xl font-bold text-foreground">
                      Track the Meta in Real Time
                    </h3>
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Win rates, W/L records, and card variant data from thousands of top-ranked battles. Updated daily.
                    </p>
                  </div>
                  <ArrowRight className="w-4 h-4 text-muted-foreground group-hover:text-primary group-hover:translate-x-0.5 transition-all shrink-0 mt-1" />
                </div>
                ...
                <div className="flex-1 space-y-2">
                  {[
                    { name: "Mortar Control", wr: 54.2, colors: ["evolved", "evolved", "hero", "hero", "normal", "normal", "normal", "normal"] },
                    { name: "Royal Hogs Cycle", wr: 51.8, colors: ["evolved", "evolved", "hero", "hero", "normal", "normal", "normal", "normal"] },
                  ].map((deck) => (
                    <div key={deck.name} className="flex items-center gap-3 px-3 py-2 bg-muted/20 border border-border/30 rounded-lg">
                      <div className="flex gap-px">
                        {deck.colors.map((v, i) => (
                          <div key={i} className={`w-4 h-5 rounded-sm ${v === "evolved" ? "bg-purple-500/60" : v === "hero" ? "bg-yellow-500/60" : "bg-muted/60"}`} />
                        ))}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-xs font-semibold text-foreground truncate">{deck.name}</div>
                      </div>
                      <div className={`text-xs font-bold tabular-nums ${deck.wr >= 53 ? "text-emerald-400" : "text-amber-400"}`}>{deck.wr}%</div>
                    </div>
                  ))}
                  <div className="grid grid-cols-3 gap-2 pt-1">
                    {[
                      ...
                      <div key={f.label} className="flex flex-col items-center gap-1 py-2 bg-muted/10 rounded-lg">
            ...
            {/* Head-to-Head */}
            <Link href="/head-to-head" className="group relative bg-card border border-border/50 rounded-2xl overflow-hidden hover:border-purple-500/30 hover:shadow-[0_0_30px_rgba(168,85,247,0.06)] transition-all duration-300 hover:-translate-y-0.5">
                ...
                <div className="flex items-start justify-between mb-5">
                  <div className="space-y-2">
                    <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-purple-500/10 border border-purple-500/30 rounded-full">
                      ...
                      <span className="text-[10px] font-semibold text-purple-400 uppercase tracking-widest">Head-to-Head</span>
                    </div>
                    <h3 className="font-[family-name:var(--font-heading)] text-xl font-bold text-foreground">
                      Compare Win Conditions
                    </h3>
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Pit any two win conditions against each other to see which dominates the current ladder.
                    </p>
                  </div>
      ...
      <section className="relative py-24 md:py-32 px-6 md:px-12 bg-card/20">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16 md:mb-20 space-y-4 opacity-0 animate-[fade-in_0.8s_ease-out_0.2s_forwards]">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Win rates, W/L records, games played, and card variant data from top ranked players.
            </p>
          </div>
                ...
                <div className="px-5 py-4 bg-muted/20 border-b border-border/30">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      ...
                      <h3 className="text-base font-bold text-foreground truncate">{deck.name}</h3>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <div className={`text-xs font-bold tabular-nums ${deck.winRate >= 53 ? "text-emerald-400" : "text-amber-400"}`}>
                        {deck.winRate}% WR
                      </div>
                      <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-pink-500/10 ring-1 ring-pink-500/20">
                        <div className="w-2 h-2 rounded-full bg-pink-500 animate-pulse" />
                        <span className="text-xs font-bold text-pink-500">{deck.avgElixir.toFixed(1)}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-5">
                  <div className="grid grid-cols-4 gap-2.5">
                    {deck.cards.slice(0, 4).map((card) => (
                      ...
                      >
                        <Image src={card.image} alt={card.name} fill className="object-contain p-0.5" />
                        {card.variant === "evolved" && (
                          <div className="absolute top-0.5 left-0.5 px-1 py-px rounded bg-purple-500/90 text-white text-[6px] font-bold uppercase leading-none z-10">Evo</div>
                        )}
                        {card.variant === "hero" && (
                          <div className="absolute top-0.5 left-0.5 px-1 py-px rounded bg-yellow-500/90 text-white text-[6px] font-bold uppercase leading-none z-10">Hero</div>
                        )}
                      </div>
                    ))}
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 opacity-0 animate-[fade-in_0.8s_ease-out_0.6s_forwards]">
            {[
              { icon: TrendingUp, title: "Win Rate Tracking", desc: "Performance stats across all deck archetypes" },
              { icon: Zap, title: "Updated Daily", desc: "Fresh data from ranked play every day" },
              { icon: Trophy, title: "Competitive Advantage", desc: "See what decks the best of the best are using" },
            ].map((feature) => (
              <div
                key={feature.title}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16 items-center">
            ...
            <div className="relative order-2 lg:order-1 opacity-0 animate-[fade-in_0.8s_ease-out_0.3s_forwards]">
              ...
              <div className="relative bg-card/80 border border-border rounded-2xl overflow-hidden backdrop-blur-sm">
                <div className="flex items-center gap-3 px-5 py-4 border-b border-border/50">
                  ...
                  <div>
                    <div className="text-sm font-semibold text-foreground">ClashGPT</div>
                ...
                <div className="p-5 space-y-4">
                  <div className="flex justify-end">
                    <div className="bg-primary/15 border border-primary/25 rounded-xl rounded-br-sm px-4 py-2.5 max-w-[80%]">
                      <p className="text-sm text-foreground">Find me the top decks with royal hogs</p>
                    </div>
                  </div>
                  <div className="bg-primary/20 border border-primary/40 text-white p-4 rounded-xl">
                    <div className="flex items-center gap-3">
                      <span className="text-lg animate-spin">⚙️</span>
                      <span className="text-sm font-medium">Finding decks for you...</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="order-1 lg:order-2 space-y-6 opacity-0 animate-[fade-in_0.8s_ease-out_0.2s_forwards]">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-card/60 border border-border/50 rounded-full backdrop-blur-sm">
                ...
                <span className="text-xs tracking-[0.2em] uppercase text-muted-foreground font-medium">AI Assistant</span>
              </div>
              <h2 className="font-[family-name:var(--font-heading)] font-bold text-4xl md:text-5xl lg:text-6xl tracking-tight">
                Ask Anything About{" "}
              ...
              <p className="text-lg text-muted-foreground max-w-lg leading-relaxed">
                A conversational AI that knows Clash Royale inside and out. Get instant answers backed by live data.
              </p>
              <div className="grid grid-cols-2 gap-3 pt-2">
                {[
                  ...
                  { icon: Crosshair, label: "Matchup Advice" },
              ...
              <div className="pt-4">
                <SignedIn>
                  <Link href="/chat" className="group inline-flex items-center gap-2 text-accent hover:text-accent/80 font-medium transition-colors">
                    Try the assistant
                    <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
                  </Link>
                </SignedIn>
      ...
      <section className="relative py-24 md:py-32 px-6 md:px-12 bg-card/20">
        ...
        <div className="relative max-w-4xl mx-auto text-center space-y-8">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-primary/10 border border-primary/20 rounded-full backdrop-blur-sm">
            ...
            <span className="text-xs tracking-[0.2em] uppercase text-primary font-medium">Join the Arena</span>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="text-center md:text-left">
              <h3 className="font-[family-name:var(--font-heading)] text-xl font-bold text-foreground mb-1">ClashGPT</h3>
              <p className="text-sm text-muted-foreground">AI-powered Clash Royale intelligence</p>
            </div>
          ...
          <div className="mt-8 pt-8 border-t border-border/50 text-center">
            <p className="text-xs text-muted-foreground/60">&copy; 2026 ClashGPT. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
